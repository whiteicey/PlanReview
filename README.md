...
面向油气田**开发方案 DOCX** 的本地自动初审工具。它把一份 Word 方案解析成带来源定位的参数事实，用一套三值规则引擎做一致性/汇总/产能/版本/术语/完整性检查，再叠加一层 LLM 复核（默认内置 Mock，可选接入在线 Anthropic 兼容模型），最后给出可追溯到原文位置的问题清单，并支持专家复核与导出。
...
其中外部权威 10 条来自示例规则包 `本地版示例数据包/rules/ruleset-demo-0.1.yaml`；仓库自有 2 条
...
scripts/        run_local.py(启动) / import_demo.py(校验外部 DOCX 并读规则)
...
python scripts/import_demo.py <a.docx> # 校验外部 DOCX + 读示例规则/术语（不复制源文件）
```
